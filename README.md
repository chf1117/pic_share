...
## 技术栈
- **前端**: HTML5, CSS3, JavaScript, Bootstrap 5, Font Awesome, qrcode.js
...
### v1.3.0 (2025-12-01)
- **点赞全链路同步**: 实现了预览模态框与列表页点赞状态的实时双向同步。现在在预览大图时点赞，列表页对应卡片的红心状态和计数会立即更新，无需刷新页面。同时重构了预览图切换逻辑，确保切换图片后点赞对象（ID）准确无误。
- **社交分享功能**: 新增了图片分享模态框。
  - **微信分享**: 集成 `qrcode.js`，实时生成当前图片链接的二维码，方便手机扫码分享。
  - **微博分享**: 支持一键跳转微博发布页并自动携带图片链接。
  - **链接复制**: 支持一键复制图片链接到剪贴板，并提供友好的交互反馈。
  - **平台引导**: 针对抖音/小红书等闭环平台，提供保存图片引导提示。
- **交互体验优化**: 修复了移动端/触屏设备上的模态框交互细节。

### v1.21 (2025-11-22)
- 启用缩略图与 WebP 系统：上传时自动生成 WebP 缩略图和完整 WebP，大图与缩略图均保持原始宽高比，原始图片路径不变。
- 缩略图规格调整：将缩略图最长边从 200px 提升到 600px，并将 WebP 质量提升到 95，兼顾 PC 多列和移动端单列浏览下的清晰度和加载性能。
- 前端展示优化：`index.html` 列表和标签搜索统一使用后端返回的 `thumbnail_url` / `webp_url` 字段，通过 `<picture>` 标签优先加载 WebP，失败时回退到原始 JPEG。
- 路径规范修复：新增 `build_image_url` 辅助函数，统一将数据库中的绝对/相对/Windows 风格路径规范化为 `/uploads/...` URL，修复标签搜索时出现 `http://var/www/pic/...` 导致图片无法加载的问题。
- 预览模态框修复：修正图片预览多次点击后不再弹出的问题，改为复用单一 Bootstrap Modal 实例并直接更新 `#previewImage.src`，保证连续预览体验稳定。
- 批量迁移增强：`scripts/migrate_existing_images.py` 增加 `--force` 参数，支持在已存在缩略图/WebP 的情况下强制重新生成，本次已使用该模式将约 2000 张历史图片的缩略图统一升级到 600/95 规格。
- 上传体验：`upload.html` 已实现详细的单文件进度条、整体进度显示及失败重试机制，上传错误不会影响其他文件，符合 v1.2 上传体验设计要求。
...
**功能需求**：
- [x] 图片上传时自动生成缩略图（最长边 600px，保持原始宽高比，WebP 格式）
- [x] 同时生成完整 WebP 格式图片
- [x] 主页/列表页使用缩略图展示
- [x] 点击后加载完整 WebP 格式图片
- [x] **原图保持在原路径不变**（兼容现有 ~2000 张图片）
- [x] 提供批量转换脚本处理现有图片
- [x] **不裁剪图片内容，保持摄影作品完整性**
...
**技术实现**：
- 缩略图格式：WebP（最长边 600px，保持宽高比，WebP 质量 95）
- 完整图格式：WebP（原尺寸，比 JPEG 小 30-35%）
- 原图位置：保持在 `uploads/` 目录（不移动）
- 缩略图位置：`uploads/thumbnails/`
- WebP 位置：`uploads/webp/`
- 缩略图示例：
  - 横向照片 4000x3000 → 600x450px
  - 竖向照片 3000x4000 → 450x600px
  - 全景照片 6000x2000 → 600x200px
...
**功能需求**：
- [x] 扫描 `uploads/` 目录下所有现有图片
- [x] 为每张图片生成缩略图和 WebP 格式
- [x] 更新数据库记录（添加新字段）
- [x] 显示处理进度和统计信息
- [x] 支持断点续传（跳过已处理的图片）
- [x] 错误处理和日志记录
